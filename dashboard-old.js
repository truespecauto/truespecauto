...
// If no token, or if an admin somehow lands here, redirect appropriately
if (!token || !user) {
    window.location.href = '/login.html';
} else if (user.role === 'admin') {
    // Phase 8 will cover the admin portal, but we route them out of the client view
    window.location.href = '/admin-portal.html'; 
}

document.getElementById('user-greeting').innerText = `HELLO, ${user.name}`;

// Prompt password change if using auto-generated temp password
                        ...
                        ${pastBookings.map(b => `
                            <tr class="hover:bg-slate-50 transition">
                                ...
                                <td class="px-6 py-4 text-right">
                                    ...
                                    `<a href="/api/data/files/${b.reportPdfKey}?token=${token}" class="text-truespec-sky font-bold hover:underline uppercase tracking-wide">Download</a>` 
                                    : '-'}
                                </td>
                            </tr>
                        `).join('')}
...
function renderActiveBookingCard(booking) {
    // Determine progress step (1 to 4)
    let step = 1;
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-slate-50 p-6 rounded-lg border border-gray-200">
                ...
                <div class="bg-white p-4 rounded border border-gray-200 mb-4 flex justify-between items-center shadow-sm">
                    ...
                    ${booking.invoicePdfKey ? `<a href="/api/data/files/${booking.invoicePdfKey}?token=${token}" class="text-xs bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-3 rounded uppercase tracking-wide transition">View Invoice</a>` : ''}
                </div>
...
async function submitPayment(e, bookingId) {
    ...
    if (!txCode && fileInput.files.length === 0) {
        ...
        btn.disabled = false;
        btn.innerText = "Submit Payment Details";
        return;
    }
    ...
    try {
        const res = await fetch(`/api/data/payment/${bookingId}`, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${token}` },
            body: formData
        });
...
// Handle Auto-generated temp password update
async function updatePassword() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Update localstorage so it doesn't prompt again
